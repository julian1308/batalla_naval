...
def dibujar_grilla():
    ...
    # Dibujar impactos y agua
    for disparo in disparos_realizados:
        fila, col, impacto = disparo
        x = x_inicial + col * TAMANIO_CASILLA + TAMANIO_CASILLA // 2
        y = y_inicial + fila * TAMANIO_CASILLA + TAMANIO_CASILLA // 2
        if impacto:
            pygame.draw.circle(pantalla, ROJO, (x, y), 10)  # Impacto
        else:
            pygame.draw.circle(pantalla, AZUL, (x, y), 10)  # Agua
...
def disparar(filas, columnas, barcos_colocados, puntaje):
    for barco, celdas in barcos_colocados.items():
        if (filas, columnas) in celdas:
            ...
            disparos_realizados.append((filas, columnas, True))  # Registrar impacto
            if len(celdas) == 0:
                puntaje += 10 * barcos[barco]  # Hundir el barco
            return puntaje, True
    puntaje -= 1  # Agua
    disparos_realizados.append((filas, columnas, False))  # Registrar agua
...
def pantalla_juego(puntaje):
    ...
    color_boton = (AMARILLO) if salir_rect.collidepoint(mouse_pos) else COLOR_BOTONES
    pygame.draw.rect(pantalla, color_boton, salir_rect)  # Dibujar botón "Salir"

    # Texto del botón "Salir"
    texto_salir = fuente.render("Salir", True, NEGRO)
    pantalla.blit(texto_salir, (salir_rect.x + 10, salir_rect.y + 5))
...
def menu_principal():
    ...
    while True:
        ...
        if jugar_rect.collidepoint(mouse_pos):
            pygame.draw.rect(pantalla, AMARILLO, jugar_rect, border_radius=5)
        else:
            pygame.draw.rect(pantalla, COLOR_BOTONES, jugar_rect, border_radius=5)
        
        if puntajes_rect.collidepoint(mouse_pos):
            pygame.draw.rect(pantalla, AMARILLO, puntajes_rect, border_radius=5)
        else:
            pygame.draw.rect(pantalla, COLOR_BOTONES, puntajes_rect, border_radius=5)
        
        if salir_rect.collidepoint(mouse_pos):
            pygame.draw.rect(pantalla, AMARILLO, salir_rect, border_radius=5)
        else:
            pygame.draw.rect(pantalla, COLOR_BOTONES, salir_rect, border_radius=5)
    ...
    while True:
        ...
        if seleccion == "jugar":
            ...
            while juego_en_curso:
                salir_rect = pantalla_juego(puntaje)
                for evento in pygame.event.get():
                    if evento.type == pygame.QUIT:
                        pygame.quit()
                        sys.exit()
                    if evento.type == pygame.MOUSEBUTTONDOWN:
                        # Detectar si se hizo clic en el botón "Salir"
                        if salir_rect.collidepoint(evento.pos):
                            juego_en_curso = False  # Salir del bucle del juego
                            break

                        # Convertir posición del clic a coordenadas de la grilla
                        fila = (evento.pos[1] - (ALTO - FILAS * TAMANIO_CASILLA) // 2) // TAMANIO_CASILLA
                        columna = (evento.pos[0] - (ANCHO - COLUMNAS * TAMANIO_CASILLA) // 2) // TAMANIO_CASILLA
                        if 0 <= fila < FILAS and 0 <= columna < COLUMNAS:  # Verificar límites
